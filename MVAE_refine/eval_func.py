...
def mae_all(result_txt_rgbd,test_data_root,model_root,test_datasets,save_film_name):
    f = open(result_txt_rgbd, 'w+')
    gt_dir=test_data_root
    # test_datasets = ["NC4K"]
    print('[INFO]: Process in Path [{}]'.format(model_root) , file=f)
    ...
    for dataset in test_datasets:
        ...
        if dataset != 'SOD':
            loader = eval_Dataset(osp.join( model_root+ '/'+save_film_name+'/'+dataset), osp.join(gt_dir, dataset, 'GT'))
        else:
            loader = eval_SODDataset(osp.join( model_root+ '/'+save_film_name+'/'+ dataset), osp.join(gt_dir, dataset, 'GT'))
